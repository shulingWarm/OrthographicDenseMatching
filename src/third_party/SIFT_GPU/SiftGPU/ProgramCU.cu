...
//how many keypoint descriptor to normalized in a lock (32, ...)
...
{
#if defined(ENABLE_LINEAR_TEX2D) && CUDA_VERSION > 2010
	cudaBindTexture2D(0, &texRef, &texRef.channelDesc, _imgWidth, _imgHeight, _imgWidth* _numChannel* sizeof(float));
#else
...
int ProgramCU::CheckCudaDevice(int device)
{
    ...
    {
        //std::cout << "using #" << device << "out of "<< count << " devices\n";
        cudaSetDevice(device);
        CheckErrorCUDA("cudaSetDevice\n"); 
    }
    cudaGetDevice(&device_used);
    if(device != device_used) fprintf(stdout, "Using # %d device out of %d\n", device_used, count);
    return 1;
}
