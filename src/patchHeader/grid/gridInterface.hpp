...
public:
    ...
    {
        return std::round((x-getDimMin(dim))/getSpatialResolution(dim));
    }

    //从离散域转成连续域
    double toSpatialDim(int dim,int x)
    {
        return (0.5+(double)x)*getSpatialResolution(dim)+getDimMin(dim);
    }

    //判断一个网格单元是否在范围内
    virtual bool isGridDimInRange(int dim,int x)
    {
        return x>=0 && x<getDimSize(dim);
    }

    //判断某个空间维度是否在范围内
    virtual bool isSpatialDimInRange(int dim,double x)
    {
        return isGridDimInRange(dim,toGridDim(dim,x));
    }

    //判断一个空间点是否在范围内
    virtual bool isSpatialPointInRange(double* xy)
    {
