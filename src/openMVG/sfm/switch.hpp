...
#define NCC_THRE 0.75 //两个图的颜色互相关性的阈值，也就是论文里面的NCC
//表示总共需要迭代的次数，这个次数会按照比例分配给每人高度阶层
#define FIX_MASK_TIMES 350//按照一个固定的大小循环若干次，而不是依次递减的窗口大小
...
#endif
//每个先验点可用次数的初始值
#define REF_TIME_INIT 3
//什么时候有必要开始判断结束循环的条件
#define CONSIDER_BREAK_TIME 250
//简单降低阈值的时间
#define FIRST_DOWN_THRE 250
//什么时候初始化所有的可用次数
inline bool NEED_INIT_REF(int id)
{
    //return id==FIRST_DOWN_THRE;
...
{
    if(id>=FIRST_DOWN_THRE) return NCC_THRE-(id-FIRST_DOWN_THRE)*0.015;
    //每20个节点是一个小周期
    return NCC_THRE;
}
//可参考的阈值与真实阈值的差值
#define REF_THRE_DIS 0.3

//每次迭代的时候允许用到的最小的z阶层，用于保证高处的场景被优先重建
//不用的时候把它写成0就可以
inline int ALLOW_MIN_Z(int id)
{
    //if(id>=FIRST_DOWN_THRE) return 199.f-(id-FIRST_DOWN_THRE)*2;
    //在第1个周期，按照序号降低
    return 199.f-id;
}
...
{
    return false;
    if(590>=xValue && 590-xValue<=3 && 478>=yValue && 478-yValue<=3)
        return true;
    return false;
}

#define PRIOR_NCC (NCC_THRE-0.05)//用于给先验点写入，让它一开始就成为一个可以使用的点
...
#define NORM_OPT_TIMES 8 //法向量的最大更新次数,超过这个次数之后如果还没找到就返回了
