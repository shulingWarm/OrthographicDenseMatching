...
///for having got gps data
bool GlobalSfM_Translation_AveragingSolver::Run_gps
(
  ETranslationAveragingMethod eTranslationAveragingMethod,
  openMVG::sfm::SfM_Data & sfm_data,
  const openMVG::sfm::Features_Provider * features_provider,
...
void GlobalSfM_Translation_AveragingSolver::Compute_translations_gps
(
  const sfm::SfM_Data & sfm_data,
  const sfm::Features_Provider * features_provider,
  const sfm::Matches_Provider * matches_provider,
...
)
{
  ...
  // Compute relative translations over the graph of global rotations
  //  thanks to an edge coverage algorithm
  ComputePutativeTranslation_EdgesCoverage_gps(
    sfm_data,
    map_globalR,
    features_provider,
    matches_provider,
...
//for got gps data
void GlobalSfM_Translation_AveragingSolver::ComputePutativeTranslation_EdgesCoverage_gps(
...
//              sOutDirectory);
          ...
          vec_tis[0] = sfm_data.poses.at(triplet.i).translation();
          vec_tis[1] = sfm_data.poses.at(triplet.j).translation();
          vec_tis[2] = sfm_data.poses.at(triplet.k).translation();



          const bool bTriplet_estimation = true;
