...
#pragma omp parallel for
                 ...
                 IndexT stepSegNum=OCCLUDE_DETECT_SEG_NUM)
  {
        ...
        //令步长起码等于一个像素长度
//        if(eachStepLength<domInfo_.pixelLength_)
//        {
//            eachStepVec=(domInfo_.pixelLength_/eachStepLength)*eachStepVec;
//            eachStepLength=domInfo_.pixelLength_;
//            stepSegNum=heightDiff/eachStepVec[2]+1;
//        }

        //遍历步长中的每个位置
        for(int idStep=0;idStep<=(stepSegNum+1);idStep+=1)
        {
            //当前的判别位置
            Point3DBase currLocation;
            //2023-2-22 防止上下叠起来的那种地形，对相邻位置起码做一次遮挡检测
            if(idStep==0)
            {
                currLocation=domPoint+(2.0/eachStepLength)*eachStepVec;
            }
            else //正常情况下的遮挡判断位置
            {
                 currLocation=domPoint+(idStep)*eachStepVec;
            }
            //获取当前位置的DOM高度
...
//                           domInfo_.getZLevel(zValueOfCurrLocation)))
                         ...
                         FileHandler* filePtr=nullptr)
  {
        ...
        //通知三角面片缓存当前位置的数据 固定Z值的时候不需要保存这个东西
        if(!GridConfig::useCurrentHeightFlag())
            srcPatch.saveData();
        //最大迭代次数
        unsigned maxOptTime=NORM_OPT_TIMES;
        //对于固定Z值的情况只迭代一次就行
        if(GridConfig::useCurrentHeightFlag()) maxOptTime=1;
        //遍历必要的次数，寻找一个合适的ncc分数
        for(unsigned int iterTime=0;iterTime<maxOptTime;++iterTime)
        {
            //获取面片在每个位置上的投影颜色
            ...
            if(currColor.patchList_.size()>=NCC_MIN_IMG || GridConfig::useCurrentHeightFlag()==1)
            {
                //根据目前的颜色情况计算ncc分数
#ifdef USE_KMEANS
...
#endif
                //记录最大的分数
                if(tempScore>maxNcc || GridConfig::useCurrentHeightFlag()==1)
                {
                    maxNcc=tempScore;
                    //通知三角面片记录当前的数据情况
                    if(!GridConfig::useCurrentHeightFlag())
                    ...
                    //判断是否有必要记录颜色和法向量
                    if(findMax || GridConfig::useCurrentHeightFlag()==1)
                    {
                        ...
                        if(holdZValue_ || GridConfig::useCurrentHeightFlag())
                        {
  ...
  void occlusionSelect(DomRectPatch& srcPatch,FileHandler* filePtr=nullptr)
  {
      ...
      //判断遮挡时用的步长
      unsigned occludeStepNum=OCCLUDE_DETECT_SEG_NUM;
      //最终决断的时候使用更大的步长
      if(GridConfig::useCurrentHeightFlag())
      {
          //将步长增大
      ...
      for(IndexT idLine=0;idLine<centerZOpt.viewLines_.size();++idLine)
      {
          ...
          //判断是否存在遮挡
          if(isOcclude(centerPoint,currProjLine.imgId_,occludeStepNum))
          {
              //把当前直线标记成不可用
    ...
    )
  {
      ...
      //如果使用固定z值的方法就只迭代一次
      if(GridConfig::useCurrentHeightFlag()) zStepMax=1;
      for(uint zId=0;zId<zStepMax;++zId)
      {
            //把平面设置为指定的值
            //如果当前位置不可用，则跳过该位置
          if(!GridConfig::useCurrentHeightFlag())
            ...
            try {
                ...
                else//2023-2-22 固定所有z值的情况下优化一次就行了
                {
                    return true;
                }
                //如果优化成功就停止
  ...
  void getZAsCloud(double scoreLimit=0)
  {
      ...
      for(unsigned int rowCount=0;rowCount<domInfo_.domHeight_;++rowCount)
      {
          for(unsigned int colCount=0;colCount<domInfo_.domWidth_;++colCount)
          {
              ...
              //没优化出来的点就不用了
              if(getUnit.nccScore_<scoreLimit) continue;
...
#pragma omp parallel for
          ...
          for(unsigned int domCol=bgStep;domCol<=domWidth;domCol+=maskSize)
          {
              ...
              {
                  //判断是否存在未优化的点
                  //2023-2-22 如果使用固定z值高度的方法，不需要做这个判断
                  if(!GridConfig::useCurrentHeightFlag())
