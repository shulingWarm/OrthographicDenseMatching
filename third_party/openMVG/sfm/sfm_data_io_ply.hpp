      ...
      if (b_structure)
      {
        ...
        for ( const auto & iterLandmarks : landmarks )
        {
          ...
          else
          {
            ...
            //为了输出颜色做的魔改
            if(iterLandmarks.second.avgInited)
            {
                //获取具体的颜色
                const cv::Vec3i& thisColor=iterLandmarks.second.avgColor;
                stream.write( reinterpret_cast<const char*> ( Vec3uc(thisColor[2], thisColor[1], thisColor[0]).data() ), sizeof( Vec3uc ) );
            }
            else
