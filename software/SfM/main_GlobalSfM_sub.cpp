...
//    << "------------------------------------------------------------"
  ...
  std::string sIntrinsic_refinement_options = "ADJUST_DISTORTION";
...
//  cmd.add( make_option('M', sSubMatchIdList, "subMatchIdList") );
  ...
  cmd.add( make_option('g', bNeedSixteenth, "needSixteenth") );
  ...
  SfM_Data sfm_data;
  if (!Load(sfm_data, sSfM_Data_Filename, ESfM_Data(ALL))) { //ESfM_Data(VIEWS|INTRINSICS|CONTROL_POINTS))) { ///!!!
    std::cerr << std::endl
  ...
  std::cout << "Process_step21" << std::endl;
  if (sfmEngine.Process_step21())//_GCP_GPS(rtFilePath))//_GCP())//_threeCameras_imu_gps_init_new(rtFilePath))//Process())//Process_threeCameras_imu_gps(sMatchesDir))//Process())
  {
    ...
    Save(sfmEngine.Get_SfM_Data(),
      stlplus::create_filespec(sOutDir, "sfm_data_s_"+s, ".bin"),
      ESfM_Data(ALL));

    Save(sfmEngine.Get_SfM_Data(),
      stlplus::create_filespec(sOutDir, "sfm_data_s_"+s, ".json"),
      ESfM_Data(ALL));
