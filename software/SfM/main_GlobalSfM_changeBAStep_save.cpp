...
//  }
  ...
  std::cout << "bNeedSixteenth " << bNeedSixteenth << std::endl;
  if(bNeedSixteenth)
  {
...
//      sfmEngine.Set_Intrinsics_Refinement_Type(intrinsic_refinement_options);
//      sfmEngine.Process_GCP_GPS(sOutDir+"/n_gps_imu_321_3.res");
//      Save(sfmEngine.Get_SfM_Data(),
//        stlplus::create_filespec(sOutDir, "sfm_data_test_save", ".ply"),
//        ESfM_Data(ALL));
  }

  if (sfmEngine.Process_GCP_GPS_BAStep_save(rtFilePath, sOutDir, iBId))//Process_threeCameras_imu_gps_init_new())//Process_test(sMatchesDir))//_down_imu_gps(sMatchesDir))
  {
      Save(sfmEngine.Get_SfM_Data(),
        stlplus::create_filespec(sOutDir, "sfm_data_save", ".ply"),
        ESfM_Data(ALL));
      //
