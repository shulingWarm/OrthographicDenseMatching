...
//    << "------------------------------------------------------------"
  ...
  //see tracks
//  const Pair_Set pairs = matches_provider_all->getPairs();
//  matches_provider->pairWise_matches_;
//  matching::PairWiseMatches m;
//  std::cout << "size : " << matches_provider->pairWise_matches_.size() << std::endl;
//  for (const auto & match_info :  matches_provider->pairWise_matches_)
//  {
////    const View * vI = sfmEngine.Get_SfM_Data().at(match_info.first.first).get();
////    const View * vJ = sfmEngine.Get_SfM_Data().at(match_info.first.second).get();
//      if (match_info.first.first == 221912 )
//      {
//          std::cout << "2 " <<  match_info.first.second << std::endl;
//      }

//    if (match_info.first.first == 210246 && match_info.first.second == 221912)
//    {
...
//  return EXIT_SUCCESS;
  ...
  if (sfmEngine.Process_GCP_GPS(rtFilePath))//Process_threeCameras_imu_gps_init_new())//Process_test(sMatchesDir))//_down_imu_gps(sMatchesDir))
  {
