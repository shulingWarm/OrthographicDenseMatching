...
int main(int argc, char **argv)
{
  ...
  if (filter_ptr)
  {
    ...
    switch (eGeometricModelToCompute)
    {
      ...
      {
        filter_ptr->Robust_model_estimation(GeometricFilter_EMatrix_AC(4.0, imax_iteration),//(GeometricFilter_EMatrix_AC(4.0, imax_iteration),
          map_PutativesMatches, bGuided_matching, d_distance_ratio, &progress);
        map_GeometricMatches = filter_ptr->Get_geometric_matches();
        std::cout << "map_GeometricMatches : " << map_GeometricMatches.size() << std::endl;
